...
{
  if (m_szSelectedText != NULL) delete m_szSelectedText;
  BSTR selectedTextW = getSelectionFunc(gettext);
  if (selectedTextW == NULL || wcslen(selectedTextW) == 0) return NULL;
  ...
  if (SUCCEEDED(pcmdTarget->QueryInterface(IID_IOleCommandTarget, (void**)&pOleCommandTarget))) 
  {
    if (SUCCEEDED(pOleCommandTarget->QueryInterface(IID_IOleWindow, (void**)&pOleWindow))) 
    {
      ...
      if (m_pCtrl->m_pPopupMenuListener != NULL)
      {
        ...
        if (dwID == 5) type = PopupMenuListener::MenuType::Anchor;
        else if (dwID == 4) type = PopupMenuListener::MenuType::Selection;
        else if (dwID == 1) type = PopupMenuListener::MenuType::Image;
