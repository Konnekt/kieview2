...
public:
  ...
  static cMessage prepare(const StringRef& to, const StringRef& from, int net, 
    const StringRef& body, int type, const StringRef& ext = "", int flag = 0);
  ...
  static void send(tCntId cnt, const StringRef& from, const StringRef& body, int type = MT_MESSAGE, 
    const StringRef& ext = "", bool html = false, bool inject = false);
  ...
  static void quickEvent(tCntId cnt, const StringRef& body, bool html = false, bool showTime = false, 
    bool warning = false, StringRef ext = "");

  static void inject(cMessage* msg, tCntId cnt, const char* display = 0, 
    bool scroll = true);
};
